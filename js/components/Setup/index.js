		...
		class Setup extends Component {
			constructor(props) {
				...
				this.state = {
					...
					opacity: Array(config.length)
						.fill(0)
						.map((x, i) => (i ? new Animated.Value(0) : new Animated.Value(1)))
				};
			}
			...
			navigate(index) {
				if (index < config.length && index >= 0 && index != this.state.from) {
					...
					Animated.timing(this.state.opacity[this.state.from], {
						toValue: 0
					}).start(() => {
						Animated.timing(this.state.opacity[this.state.to], {
							toValue: 1
						}).start(() => {
			...
			render() {
				...
				return (
					<View style={{ flex: 1, backgroundColor: theme.bgColor }}>
						{config.map(
							(child, idx) =>
								(idx === this.state.from || idx === this.state.to
									? <Animated.View
											key={idx}
											style={{
										...
										>
											{React.createElement(child, {
												navigateNext: () => this.navigate(idx + 1)
											})}
										</Animated.View>
									: null)
						)}
